...
- Verify JSON formatting and aligned identity across the Copilot (`plugin.json`), Codex (`.codex-plugin/plugin.json`), and Claude (`.claude-plugin/plugin.json`) manifests, canonical catalog, and marketplace indexes.
...
- [ ] The package contains valid Copilot, Codex, and Claude manifests plus the command definitions in `commands/` and `com.github.copilot/commands/`.
